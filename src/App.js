...
function getShipGradient(style) {
  if (style === "VOID")
    return "radial-gradient(circle at 28% 24%, #ffffff 0%, #a78bfa 22%, #7c3aed 52%, #1e063f 100%)";
  if (style === "SOLAR")
    return "radial-gradient(circle at 28% 24%, #fff7ed 0%, #fed7aa 22%, #fb923c 48%, #ef4444 72%, #450a0a 100%)";
  if (style === "NEBULA")
    return "radial-gradient(circle at 25% 22%, #ffffff 0%, #f0abfc 18%, #ec4899 38%, #06b6d4 68%, #111827 100%)";
  if (style === "GOLD")
    return "radial-gradient(circle at 28% 22%, #ffffff 0%, #fef08a 20%, #facc15 48%, #b45309 76%, #451a03 100%)";
  if (style === "GLITCH")
    return "linear-gradient(135deg, #ffffff 0%, #22d3ee 18%, #111827 34%, #f472b6 52%, #22d3ee 68%, #030712 100%)";
  if (style === "ABYSSAL")
    return "radial-gradient(circle at 50% 50%, #000000 0%, #020617 38%, #050510 72%, #000000 100%)";
...
function getBoostGradient(style) {
  if (style === "VOID")
    return "linear-gradient(to left, rgba(124,58,237,1), rgba(192,132,252,0.95), rgba(255,255,255,0.55), transparent)";
  if (style === "SOLAR")
    return "linear-gradient(to left, rgba(239,68,68,1), rgba(251,146,60,1), rgba(254,240,138,0.95), transparent)";
  if (style === "ICE")
    return "linear-gradient(to left, rgba(14,165,233,1), rgba(186,230,253,1), rgba(255,255,255,0.75), transparent)";
  if (style === "PLASMA")
    return "linear-gradient(to left, rgba(236,72,153,1), rgba(34,211,238,1), rgba(255,255,255,0.7), transparent)";
  if (style === "RAINBOW")
    return "linear-gradient(to left, #ef4444, #f97316, #facc15, #22c55e, #38bdf8, #a78bfa, transparent)";
  if (style === "ABYSSAL")
    return "linear-gradient(to left, rgba(0,0,0,1), rgba(20,20,30,0.95), rgba(80,40,120,0.35), transparent)";
...
function getBoostCoreGradient(style) {
  if (style === "VOID")
    return "linear-gradient(to left, rgba(255,255,255,1), rgba(216,180,254,0.95), transparent)";
  if (style === "SOLAR")
    return "linear-gradient(to left, rgba(255,255,255,1), rgba(254,240,138,1), transparent)";
  if (style === "ICE")
    return "linear-gradient(to left, rgba(255,255,255,1), rgba(186,230,253,1), transparent)";
  if (style === "PLASMA")
    return "linear-gradient(to left, rgba(255,255,255,1), rgba(125,211,252,1), transparent)";
  if (style === "RAINBOW")
    return "linear-gradient(to left, rgba(255,255,255,1), rgba(250,204,21,0.95), rgba(56,189,248,0.85), transparent)";
  if (style === "ABYSSAL")
    return "linear-gradient(to left, rgba(180,120,255,0.75), rgba(0,0,0,0.95), transparent)";
...
export default function App() {
  ...
  const [previewShipStyle, setPreviewShipStyle] = useState(() =>
    cleanChoice(safeRead("homebound_shipStyle", "DEFAULT"), "DEFAULT")
  );
  const [previewBoostStyle, setPreviewBoostStyle] = useState(() =>
    cleanChoice(safeRead("homebound_boostStyle", "FIRE"), "FIRE")
  );
  const [previewDeathFx, setPreviewDeathFx] = useState(() =>
    cleanChoice(safeRead("homebound_deathFx", "STATIC"), "STATIC")
  );
  const [previewAuraStyle, setPreviewAuraStyle] = useState(() =>
  ...
  const nextPhaseScore =
    phaseStartScores[Math.min(phaseIndex + 1, phaseStartScores.length - 1)];
  const distanceToNext = Math.max(0, nextPhaseScore - score);
  const phaseBleed =
    phaseIndex < phases.length - 1 && distanceToNext <= 8
      ? (8 - distanceToNext) / 8
      : 0;
  const phasePulse =
    phaseBleed > 0
      ? 0.2 + phaseBleed * 0.28 + Math.abs(Math.sin(score * 1.7)) * 0.16
      : 0;
  const isTranscendent = currentPhase.name === "TRANSCENDENCE";
  const transcendenceOverflow = Math.max(0, score - 150);
  const wallSpeed = isTranscendent
    ? currentPhase.speed + transcendenceOverflow * 0.035
    : currentPhase.speed;
  const glowSize = TAP_TO_PLAY_MODE
    ? Math.min(
        90,
        isTranscendent
          ? currentPhase.glow + transcendenceOverflow * 0.06
          : currentPhase.glow
      )
    : isTranscendent
    ? currentPhase.glow + transcendenceOverflow * 0.12
    : currentPhase.glow;
  const instabilityOffset =
    currentPhase.name === "INSTABILITY" ? Math.sin(score * 0.9) * 10 : 0;
  ...
  const stars = useMemo(
    () =>
      Array.from({ length: isTranscendent ? 16 : 20 }, (_, i) => ({
        id: i,
        left: (i * 73) % 100,
        top: (i * 37) % 100,
        size: i % 3 === 0 ? 2 : 1,
  ...
  function endRun(finalScore) {
    const adjustedScore = continuedRun
      ? Math.floor(finalScore * 0.65)
      : finalScore;
    ...
    setStats((s) => ({
      ...
      bestPhase:
        phaseStartScores[getPhaseIndex(adjustedScore)] >=
  ...
  function revive() {
    ...
    setShipY(GAME_H / 2);
    setWalls((w) =>
      w.filter((wall) => wall.x < SHIP_X - 90 || wall.x > SHIP_X + 170)
    );
  ...
  useEffect(() => {
    ...
    const loop = setInterval(
      () => {
        setVelocity((v) =>
          Math.max(v + GRAVITY + (holdingBoost ? -1.05 : 0), -6.6)
        );
        ...
        if (spawnTimer.current > WALL_SPAWN_FRAMES) {
          ...
          if (Math.abs(gapY - lastGapY.current) < 55) {
            gapY =
              lastGapY.current < GAME_H / 2
                ? maxGapY - Math.random() * 80
                : minGapY + Math.random() * 80;
          }

          lastGapY.current = gapY;
          setWalls((w) => [
            ...w,
            { id: Date.now() + Math.random(), x: GAME_W, gapY, scored: false },
          ]);
        }
            ...
            .map((wall) => {
              const moved = {
                ...
                x:
                  wall.x -
                  wallSpeed +
                  (currentPhase.name === "INSTABILITY"
                    ? Math.sin((wall.x + score) * 0.03) * 1.4
                    : 0),
              };
              ...
              const touchingX =
                shipRight > moved.x && shipLeft < moved.x + WALL_WIDTH;
              const touchingY =
                shipTop < moved.gapY - GAP / 2 ||
                shipBottom > moved.gapY + GAP / 2;

              if (touchingX && touchingY && !deathFocus && !invulnerable) {
                const meaningfulDeath =
                  score > 40 || score >= best || isTranscendent;
                if (meaningfulDeath) {
                  playSound("death");
                  stopAmbience();
                  stopBoostSound();
              ...
              const nearMiss =
                touchingX &&
                !touchingY &&
                (Math.abs(shipTop - (moved.gapY - GAP / 2)) < 18 ||
                  Math.abs(shipBottom - (moved.gapY + GAP / 2)) < 18);
              ...
              if (nearMiss) {
                ...
                setNearMissSparks((s) => [
                  ...s.slice(-8),
                  {
                    ...
                    x: SHIP_X + SHIP_SIZE + 8,
                    y: shipY + SHIP_SIZE / 2,
                    color: currentPhase.accent,
                  },
                ]);
                setTimeout(() => setNearMissFlash(false), 90);
                setTimeout(() => setNearMissSparks((s) => s.slice(1)), 260);
              }

              if (
                !scoredWallIds.current.has(moved.id) &&
                moved.x + WALL_WIDTH < SHIP_X - 10
              ) {
        ...
        <>
          <div
            style={{
              position: "absolute",
              left: wall.x - 42,
              top: bottom ? top - 30 : top + height - 30,
              width: WALL_WIDTH + 84,
              height: 60,
              borderRadius: 999,
              border: `2px solid ${stationGlow}`,
              transform: `rotate(${
                bottom ? -18 : 18
              }deg) translateY(${ringOffset}px)`,
              filter: "blur(0.3px)",
              opacity: 0.4,
            }}
          />
          <div
            style={{
              ...
              height,
              background:
                "linear-gradient(180deg, rgba(255,240,180,0.98), rgba(255,170,60,0.95) 30%, rgba(255,90,20,0.92) 100%)",
              borderRadius: 42,
              boxShadow: "0 0 55px rgba(255,140,40,0.85)",
              border: "2px solid rgba(255,240,180,0.55)",
              overflow: "hidden",
            }}
          >
            <div
              style={{
                ...
                opacity: 0.4,
                background:
                  "repeating-linear-gradient(135deg, rgba(255,255,255,0.22) 0px, rgba(255,255,255,0.22) 6px, transparent 6px, transparent 18px)",
              }}
            />
          </div>
          <div
            style={{
              position: "absolute",
              left: wall.x - 26,
              top: bottom ? top - 20 : top + height - 20,
              width: WALL_WIDTH + 52,
              height: 40,
              borderRadius: 999,
              background:
                "linear-gradient(to right, rgba(255,210,120,1), rgba(255,150,50,0.95), rgba(255,210,120,1))",
              boxShadow: "0 0 35px rgba(255,180,80,0.95)",
        ...
        <>
          <div
            style={{
              position: "absolute",
              left: wall.x - 34,
              top: bottom ? top - 22 : top + height - 22,
              width: WALL_WIDTH + 68,
              height: 44,
              borderRadius: 999,
              border: "2px solid rgba(255,120,120,0.35)",
              transform: `rotate(${bottom ? -22 : 22}deg)`,
              filter: "blur(2px)",
              opacity: 0.2,
            }}
          />
          <div
            style={{
              ...
              height,
              background:
                "linear-gradient(180deg, rgba(255,120,120,0.98), rgba(90,0,0,0.98))",
              clipPath: bottom
                ? "polygon(0% 0%, 76% 0%, 100% 12%, 88% 100%, 22% 92%, 0% 72%)"
                : "polygon(18% 0%, 100% 0%, 100% 84%, 82% 100%, 0% 100%, 0% 12%)",
              boxShadow: "0 0 38px rgba(255,80,80,0.78)",
              transform: `skew(${Math.sin(wall.x * 0.03) * 6}deg)`,
        ...
        <>
          ...
          <div
            style={{
              ...
              height,
              background:
                "linear-gradient(180deg, rgba(255,255,255,0.98), rgba(120,0,0,0.92))",
              clipPath: "polygon(12% 0%, 100% 8%, 82% 100%, 0% 92%)",
              boxShadow: "0 0 42px rgba(255,255,255,0.45)",
              transform: `skew(${Math.sin(wall.x * 0.02) * 4}deg)`,
            }}
          />
        </>
      );
        ...
        <>
          <div
            style={{
              ...
              height,
              background:
                "linear-gradient(180deg, rgba(219,234,254,0.22) 0%, rgba(248,250,252,0.18) 18%, rgba(199,210,254,0.2) 46%, rgba(219,234,254,0.16) 100%)",
              borderRadius: 42,
              border: "1px solid rgba(0,0,0,0.42)",
              boxShadow: "inset 0 0 28px rgba(0,0,0,0.28)",
              transform: `skew(${Math.sin(wall.x * 0.02) * 4}deg) scaleY(${
                1 + Math.sin(wall.x * 0.015) * 0.04
              })`,
              overflow: "hidden",
            }}
          >
            <div
              style={{
                ...
                inset: 0,
                background:
                  "repeating-linear-gradient(180deg, rgba(255,255,255,0.03) 0px, rgba(255,255,255,0.03) 10px, transparent 10px, transparent 30px)",
                opacity: 0.18,
              }}
            />
          </div>
          ...
          <div
            style={{
              ...
              borderRadius: 999,
              background:
                "linear-gradient(to right, transparent, rgba(0,0,0,0.22), rgba(255,255,255,0.04), transparent)",
              boxShadow: "none",
              opacity: 0.16,
            }}
          />
      ...
      <>
        <div
          style={{
            ...
            height,
            background:
              currentPhase.name === "DRIFT"
                ? "linear-gradient(160deg, rgba(115,130,160,0.95), rgba(35,45,70,0.98))"
                : currentPhase.name === "WARP"
                ? "linear-gradient(160deg, rgba(80,180,255,0.95), rgba(15,45,90,0.98))"
                : "linear-gradient(160deg, rgba(185,90,190,0.95), rgba(65,25,80,0.98))",
            clipPath: bottom
              ? "polygon(12% 0%, 78% 0%, 100% 18%, 88% 100%, 22% 92%, 0% 70%)"
              : "polygon(0% 8%, 82% 0%, 100% 32%, 76% 100%, 18% 88%, 6% 44%)",
            boxShadow: `0 0 24px ${currentPhase.accent}66`,
          }}
        />
        <div
          style={{
            ...
            top: bottom
              ? top + 18 + debrisOffset
              : Math.max(10, top + height - 52 + debrisOffset),
            width: 22,
            height: 22,
            borderRadius: 999,
            background: "rgba(255,255,255,0.28)",
            boxShadow: `0 0 16px ${currentPhase.accent}`,
            opacity: 0.75,
          }}
        />
        <div
          style={{
            ...
            top: bottom
              ? top + 54 - debrisOffset
              : Math.max(24, top + height - 96 - debrisOffset),
            width: 26,
            height: 8,
            borderRadius: 999,
            background: `linear-gradient(to right, transparent, ${currentPhase.accent}, transparent)`,
            boxShadow: `0 0 12px ${currentPhase.accent}`,
            opacity: 0.16,
          }}
        />
      </>
  ...
  function Ship({ crashed = false }) {
    ...
    return (
      <div
        style={{
          position: "relative",
          width: SHIP_SIZE * scale,
          height: SHIP_SIZE * scale,
        }}
      >
        {!crashed && isFancy && (
          <div
            style={{
              position: "absolute",
              left: SHIP_SIZE * -0.04 * scale,
              top: SHIP_SIZE * 0.11 * scale,
              width: SHIP_SIZE * 0.98 * scale,
              height: SHIP_SIZE * 0.7 * scale,
              borderRadius: 999,
              background:
                shipStyle === "GOLD"
                  ? "radial-gradient(circle, rgba(250,204,21,0.38), transparent 68%)"
                  : shipStyle === "SOLAR"
                  ? "radial-gradient(circle, rgba(249,115,22,0.36), transparent 68%)"
                  : shipStyle === "NEBULA"
                  ? "radial-gradient(circle, rgba(34,211,238,0.32), rgba(236,72,153,0.18), transparent 70%)"
                  : "radial-gradient(circle, rgba(167,139,250,0.32), transparent 68%)",
              filter: TAP_TO_PLAY_MODE ? "blur(3px)" : "blur(8px)",
              opacity: 0.9,
              pointerEvents: "none",
            }}
          />
        ...
        <div
          style={{
            position: "absolute",
            left: SHIP_SIZE * 0.06 * scale,
            top: SHIP_SIZE * 0.18 * scale,
            width: SHIP_SIZE * 0.82 * scale,
            height: SHIP_SIZE * 0.52 * scale,
            borderRadius: "52% 48% 50% 50% / 62% 58% 48% 52%",
            background: bodyStyle,
            border: crashed
              ? "2px solid rgba(255,255,255,0.35)"
              : invulnerable
              ? "2px solid rgba(34,211,238,1)"
            ...
            boxShadow: crashed
              ...
              : `0 0 ${18 + transcendenceOverflow * 0.05}px ${
                  currentPhase.accent
                }`,
            opacity:
              !crashed && invulnerable
                ? 0.72 + Math.abs(Math.sin(score * 2)) * 0.28
                : 1,
            overflow: "hidden",
          }}
        >
          {!crashed && ["NEBULA", "GOLD", "GLITCH"].includes(shipStyle) && (
            <div
              style={{
                ...
                height: 46 * scale,
                background:
                  "linear-gradient(to right, transparent, rgba(255,255,255,0.72), transparent)",
                animation: "skinShimmer 1.8s ease-in-out infinite",
                pointerEvents: "none",
              }}
            />
          )}
        ...
        {shipStyle === "ABYSSAL" && !crashed && (
          <>
            <div
              style={{
                position: "absolute",
                left: SHIP_SIZE * -0.18 * scale,
                top: SHIP_SIZE * 0.06 * scale,
                width: SHIP_SIZE * 1.18 * scale,
                height: SHIP_SIZE * 0.9 * scale,
                borderRadius: 999,
                border: "1px solid rgba(120,80,180,0.12)",
                filter: "blur(1px)",
                opacity: 0.45,
                animation: "haloOrbit 6s linear infinite",
              }}
            />
            <div
              style={{
                position: "absolute",
                left: SHIP_SIZE * 0.22 * scale,
                top: SHIP_SIZE * 0.28 * scale,
                width: SHIP_SIZE * 0.12 * scale,
                height: SHIP_SIZE * 0.12 * scale,
                borderRadius: 999,
        ...
        {shipStyle === "GLITCH" && !crashed && (
          <div
            style={{
              position: "absolute",
              left: SHIP_SIZE * 0.08 * scale,
              top: SHIP_SIZE * 0.2 * scale,
              width: SHIP_SIZE * 0.78 * scale,
              height: SHIP_SIZE * 0.08 * scale,
              background: "rgba(34,211,238,0.75)",
              opacity: 0.16,
              transform: `translateY(${Math.sin(score * 1.7) * 6}px)`,
            }}
          />
        )}
        {shipStyle === "GOLD" && (
          <div
            style={{
              position: "absolute",
              left: SHIP_SIZE * 0.2 * scale,
              top: SHIP_SIZE * 0.23 * scale,
              width: SHIP_SIZE * 0.28 * scale,
              height: SHIP_SIZE * 0.08 * scale,
              borderRadius: 999,
        ...
        <div
          style={{
            position: "absolute",
            left: SHIP_SIZE * 0.28 * scale,
            top: SHIP_SIZE * 0.29 * scale,
            width: SHIP_SIZE * 0.5 * scale,
            height: SHIP_SIZE * 0.24 * scale,
            borderRadius: 999,
            background:
              "linear-gradient(180deg, rgba(60,190,255,0.95), rgba(5,22,55,0.98) 55%, rgba(0,8,25,0.98))",
            boxShadow:
              "0 0 10px rgba(80,190,255,0.85), inset 0 2px 4px rgba(255,255,255,0.65)",
        ...
        <div
          style={{
            position: "absolute",
            left: SHIP_SIZE * -0.08 * scale,
            top: SHIP_SIZE * 0.32 * scale,
            width: SHIP_SIZE * 0.24 * scale,
            height: SHIP_SIZE * 0.2 * scale,
            borderRadius: 999,
            background:
      ...
      >
        <div
          ...
          style={{
            ...
            aspectRatio: `${GAME_W} / ${GAME_H}`,
            margin: "auto",
            transition: deathFocus
              ? "transform 850ms cubic-bezier(0.16, 1, 0.3, 1)"
              : "none",
            transformOrigin:
              deathFocus && deathZoom
                ? `${deathZoom.x}px ${deathZoom.y}px`
                : "center center",
            transform:
              deathFocus && deathZoom
                ? `scale(1.45) translate(${
                    -(deathZoom.x - GAME_W / 2) * 0.22
                  }px, ${-(deathZoom.y - GAME_H / 2) * 0.18}px)`
                : `translate(${
                    Math.sin(score * 0.15) * currentPhase.shake +
                    instabilityOffset -
                    boostKick * 2.2
                  }px, ${Math.cos(score * 0.12) * currentPhase.shake}px)`,
          }}
        >
          <style>{`
            ${GLOBAL_TOUCH_CSS}
          ...
          <div
            style={{
              position: "absolute",
              inset: 0,
              background: `linear-gradient(to bottom, ${currentPhase.colors[0]}, ${currentPhase.colors[1]}, ${currentPhase.colors[2]})`,
            }}
          />
          {isTranscendent && (
            <>
              ...
              <div
                style={{
                  ...
                  borderRadius: 9999,
                  background:
                    "radial-gradient(circle, rgba(255,255,255,0.3) 0%, rgba(191,219,254,0.14) 24%, rgba(255,255,255,0.04) 48%, transparent 72%)",
                  filter: "blur(12px)",
                  opacity: 0.9,
                  pointerEvents: "none",
                }}
          ...
          {phaseShock && (
            <div
              style={{
                ...
                opacity: PERFORMANCE_MODE ? 0.08 : 0.18,
                transform: "scale(1.8)",
                filter: PERFORMANCE_MODE ? "none" : "blur(2px)",
                pointerEvents: "none",
              }}
            />
          )}
          ...
          <div
            style={{
              ...
              opacity: 0.4,
              pointerEvents: "none",
              transform: isTranscendent
                ? `translateX(${transcendenceDrift}px)`
                : "none",
              transition: isTranscendent ? "transform 0.3s linear" : "none",
            }}
          >
            {stars.map((star) => (
              <div
                ...
                style={{
                  ...
                  background: "white",
                  left: `${
                    (star.left - score * star.depth * 0.08 + 120) % 120
                  }%`,
                  top: `${star.top}%`,
                  width: star.size,
                  height: star.size,
          ...
          {!isTranscendent && (
            <div
              style={{
                ...
                overflow: "hidden",
                pointerEvents: "none",
                opacity: currentPhase.name === "DRIFT" ? 0.5 : 0.35,
              }}
            >
              {backgroundWisps.map((wisp) => (
                <div
                  ...
                  style={{
                    position: "absolute",
                    left: `${
                      ((wisp.left - score * wisp.drift + 120) % 120) - 10
                    }%`,
                    top: `${wisp.top}%`,
                    width: wisp.width,
                    height: wisp.height,
                    ...
                    filter: TAP_TO_PLAY_MODE ? "blur(14px)" : "blur(24px)",
          ...
          {(currentPhase.name === "WARP" ||
            currentPhase.name === "VELOCITY" ||
            currentPhase.name === "SOLAR" ||
            currentPhase.name === "INSTABILITY" ||
            currentPhase.name === "FRACTURE") && (
            <div
              style={{
                position: "absolute",
                inset: 0,
                overflow: "hidden",
                pointerEvents: "none",
                opacity: holdingBoost ? 0.34 : 0.18,
              }}
            >
              {speedLines.map((line) => (
                <div
                  ...
                  style={{
                    position: "absolute",
                    left: `${
                      ((line.left - score * line.depth * 0.45 + 140) % 140) - 20
                    }%`,
                    top: `${line.top}%`,
                    width: line.width,
                    height: 1,
          ...
          {isTranscendent && (
            <div
              style={{
                ...
                borderRadius: 9999,
                background:
                  "radial-gradient(circle, rgba(255,255,255,0.52) 0%, rgba(255,255,255,0.24) 22%, transparent 68%)",
                filter: "blur(18px)",
                opacity: 0.7,
                pointerEvents: "none",
              }}
          ...
          <div
            style={{
              ...
              opacity: isTranscendent ? 0.05 : 0.5,
              pointerEvents: "none",
            }}
          />
          ...
          {comets.map((comet) => (
            <div
              ...
              style={{
                ...
                height: comet.size + 2,
                borderRadius: 999,
                background: `linear-gradient(to left, ${comet.color}, rgba(255,255,255,0.35), transparent)`,
                filter: "blur(1px)",
                opacity: 0.16,
                transform: "rotate(-12deg)",
                pointerEvents: "none",
          ...
          {fractureGhost &&
            walls.map((wall) => (
              <React.Fragment key={`ghost-${wall.id}`}>
                {renderStructure(
                  { ...wall, x: wall.x + 10 },
                  0,
                  wall.gapY - GAP / 2
          ...
          {walls.map((wall) => (
            <React.Fragment key={wall.id}>
              {renderStructure(wall, 0, wall.gapY - GAP / 2)}
              {renderStructure(wall, wall.gapY + GAP / 2, GAME_H, true)}
            </React.Fragment>
          ))}
          ...
          <div
            style={{
              position: "absolute",
              left: SHIP_X - glowSize / 2 + SHIP_SIZE / 2,
              top: shipY - glowSize / 2 + SHIP_SIZE / 2,
              width: glowSize,
              height: glowSize,
              borderRadius: 999,
              background:
                "radial-gradient(circle, rgba(255,255,255,0.22) 0%, rgba(255,255,255,0.12) 40%, transparent 75%)",
              filter: TAP_TO_PLAY_MODE
                ? "blur(3px)"
                : PERFORMANCE_MODE
                ? "blur(5px)"
          ...
          {boostFlash && (
            <>
              <div
                style={{
                  position: "absolute",
                  left: holdingBoost ? SHIP_X - 89 : SHIP_X - 21,
                  top: shipY + SHIP_SIZE / 2 - (holdingBoost ? 10 : 7),
                  width: holdingBoost ? 101 : 12,
                  height: holdingBoost ? 20 : 6,
                  borderRadius: 999,
                  ...
                  filter: TAP_TO_PLAY_MODE
                    ...
                    ? "blur(11px)"
                    : "blur(6px)",
                  opacity: holdingBoost ? 1 : 0.58,
                  transform: `rotate(${velocity * 1.1}deg)`,
                  animation: ["ICE", "PLASMA", "RAINBOW", "VOID"].includes(
                    boostStyle
                  )
                    ? "trailShimmer 0.65s ease-in-out infinite"
                    : "none",
                  pointerEvents: "none",
                }}
              />
              <div
                style={{
                  position: "absolute",
                  left: holdingBoost ? SHIP_X - 64 : SHIP_X - 14,
                  top: shipY + SHIP_SIZE / 2 - 4,
                  width: holdingBoost ? 54 : 6,
                  height: holdingBoost ? 12 : 3,
                  borderRadius: 999,
                ...
                [0, 1, 2].map((i) => (
                  <div
                    ...
                    style={{
                      position: "absolute",
                      left: SHIP_X - 72 - i * 18,
                      top: shipY + SHIP_SIZE / 2 - 10 + i * 7,
                      width: 10,
                      height: 10,
                      background: "rgba(186,230,253,0.9)",
                      clipPath: "polygon(50% 0%, 100% 50%, 50% 100%, 0% 50%)",
                      opacity: 0.55,
                      transform: `rotate(${45 + i * 18}deg)`,
                      filter: "blur(0.2px)",
                      pointerEvents: "none",
                    }}
                  />
              ...
              {holdingBoost && boostStyle === "PLASMA" && (
                <div
                  style={{
                    ...
                    animation: "trailPulse 0.55s ease-in-out infinite",
                    pointerEvents: "none",
                  }}
                />
              )}
                ...
                <>
                  <div
                    style={{
                      position: "absolute",
                      left: SHIP_X - 118,
                      top: shipY + SHIP_SIZE / 2 - 20,
                      width: 126,
                      height: 42,
                      borderRadius: 999,
                      background:
                        "linear-gradient(to left, rgba(0,0,0,1), rgba(30,30,40,0.96), rgba(88,28,135,0.45), transparent)",
                      filter: "blur(5px)",
                      opacity: 0.95,
                  ...
                  <div
                    style={{
                      position: "absolute",
                      left: SHIP_X - 84,
                      top: shipY + SHIP_SIZE / 2 - 10,
                      width: 84,
                      height: 18,
                      borderRadius: 999,
                      background:
                        "linear-gradient(to left, rgba(168,85,247,0.85), rgba(0,0,0,1), transparent)",
                      opacity: 0.92,
                      filter: "blur(1px)",
                      pointerEvents: "none",
                    }}
                ...
                <>
                  <div
                    style={{
                      ...
                      borderRadius: 999,
                      background:
                        "linear-gradient(to left, rgba(239,68,68,0.95), rgba(249,115,22,0.95), rgba(250,204,21,0.95), rgba(34,197,94,0.95), rgba(56,189,248,0.95), rgba(167,139,250,0.95), transparent)",
                      filter: "blur(2px)",
                      opacity: 0.95,
                      animation: "trailPulse 0.7s ease-in-out infinite",
                      pointerEvents: "none",
                    }}
                  />
                  ...
                  <div
                    style={{
                      position: "absolute",
                      left: SHIP_X - 82,
                      top: shipY + SHIP_SIZE / 2 - 7,
                      width: 82,
                      height: 14,
                      borderRadius: 999,
                      background:
                        "linear-gradient(to left, rgba(255,255,255,0.98), rgba(250,204,21,0.95), rgba(56,189,248,0.95), rgba(167,139,250,0.9), transparent)",
                      opacity: 0.92,
                      filter: "blur(0.5px)",
                      pointerEvents: "none",
                    }}
          ...
          <div
            style={{
              ...
              height: SHIP_SIZE,
              transform: `scale(${
                (1 + Math.min(score / 160, 0.25)) * pulseScale
              }) rotate(${velocity * 1.6}deg)`,
          ...
          >
            {phaseBleed > 0 && phaseIndex < phases.length - 1 && (
              <div
                style={{
                  fontSize: 11,
                  fontWeight: 800,
          ...
          {booted && !loading && !started && screen === "MENU" && (
            <MenuScreen
              ...
              onLaunch={(startScore) => startGame(startScore)}
      ...
      >
        ...
        <div
          style={{
            ...
            borderRadius: "52% 48% 50% 50% / 62% 58% 48% 52%",
            background:
              "radial-gradient(circle at 28% 24%, rgba(255,255,255,1) 0%, rgba(235,240,248,1) 28%, rgba(170,180,195,1) 62%, rgba(90,100,118,1) 100%)",
        ...
        <div
          style={{
            ...
            background:
              "radial-gradient(circle at 28% 24%, rgba(255,255,255,1) 0%, rgba(235,240,248,1) 28%, rgba(170,180,195,1) 62%, rgba(90,100,118,1) 100%)",
            border: "2px solid rgba(255,255,255,0.58)",
            boxShadow: "0 0 24px rgba(103,232,249,0.45)",
          }}
        />
      ...
      >
        <div
          style={{
            ...
            height: 28,
            borderRadius: 999,
            background: previewBoost,
            filter: "blur(7px)",
            opacity: 1,
            animation: "trailShimmer 0.7s ease-in-out infinite",
          }}
        />
        ...
        <div
          style={{
            ...
            height: 12,
            borderRadius: 999,
            background: getBoostCoreGradient(previewBoostStyle),
            opacity: 0.95,
          }}
        />

        {previewBoostStyle === "ICE" &&
          [0, 1, 2, 3].map((i) => (
            <div
              key={`preview-ice-${i}`}
              style={{
                ...
                boxShadow: "0 0 12px rgba(186,230,253,0.8)",
              }}
            />
          ))}
        ...
        {previewBoostStyle === "PLASMA" && (
          <div
            style={{
              ...
              background:
                "radial-gradient(circle, rgba(236,72,153,0.22), transparent 72%)",
              animation: "trailPulse 0.55s ease-in-out infinite",
            }}
          />
        )}
          ...
          <>
            <div
              style={{
                ...
                borderRadius: 999,
                background:
                  "linear-gradient(to left, rgba(0,0,0,1), rgba(20,20,28,0.96), rgba(88,28,135,0.48), transparent)",
                filter: "blur(5px)",
                opacity: 1,
                animation: "trailPulse 1.1s ease-in-out infinite",
              }}
            ...
            <div
              style={{
                ...
                borderRadius: 999,
                background:
                  "linear-gradient(to left, rgba(168,85,247,0.9), rgba(0,0,0,1), transparent)",
                opacity: 0.95,
              }}
            />
          </>
          ...
          <>
            <div
              style={{
                ...
                borderRadius: 999,
                background:
                  "linear-gradient(to left, rgba(239,68,68,0.98), rgba(249,115,22,0.98), rgba(250,204,21,0.98), rgba(34,197,94,0.98), rgba(56,189,248,0.98), rgba(167,139,250,0.98), transparent)",
                filter: "blur(3px)",
                opacity: 1,
                animation: "trailPulse 0.75s ease-in-out infinite",
              }}
            />
            ...
            <div
              style={{
                ...
                borderRadius: 999,
                background:
                  "linear-gradient(to left, rgba(255,255,255,1), rgba(250,204,21,0.98), rgba(56,189,248,0.95), rgba(167,139,250,0.92), transparent)",
                opacity: 0.95,
              }}
            />
          </>
          ...
          <>
            <div
              style={{
                ...
                boxShadow: "0 0 18px rgba(255,140,40,0.95)",
                animation: "crashFloat 1.8s ease-in-out infinite",
              }}
            />
            <div
              style={{
                ...
                boxShadow: "0 0 14px rgba(255,90,40,0.95)",
                animation: "crashFloat 2.3s ease-in-out infinite",
              }}
            />
          </>
        )}
            ...
            {previewAuraStyle === "HALO" && (
              <div
                style={{
                  ...
                  animation: "haloOrbit 3.5s linear infinite",
                }}
              />
            )}
              ...
              <>
                ...
                <div
                  style={{
                    ...
                    borderRadius: 999,
                    background:
                      "linear-gradient(to right, transparent, rgba(56,189,248,0.9), transparent)",
                    opacity: 0.9,
              ...
              <>
                <div
                  style={{
                    ...
                    borderRadius: 999,
                    background:
                      "radial-gradient(circle, rgba(0,0,0,0.96) 0%, rgba(88,28,135,0.12) 55%, transparent 78%)",
                    filter: "blur(10px)",
                    opacity: 0.95,
                    animation: "skinPulse 2.8s ease-in-out infinite",
                  }}
                />
                <div
                  style={{
                    ...
                    boxShadow: "0 0 40px rgba(88,28,135,0.22)",
                    animation: "haloOrbit 7s linear infinite reverse",
                  }}
                />
              </>
            )}
        ...
        >
          <div
            style={{
              ...
              height: 40,
              borderRadius: "52% 48% 50% 50% / 62% 58% 48% 52%",
              background: previewShip,
              border: "2px solid rgba(255,255,255,0.58)",
              boxShadow:
                previewShipStyle === "GOLD"
            ...
            {["NEBULA", "GOLD", "GLITCH"].includes(previewShipStyle) && (
              <div
                style={{
                  ...
                  animation: "skinShimmer 1.5s ease-in-out infinite",
                }}
              />
            )}
              ...
              <>
                ...
                <div
                  style={{
                    ...
                    boxShadow: "0 0 10px rgba(236,72,153,0.95)",
                  }}
                />
              </>
            )}
        ...
        >
          <div
            style={{
              ...
              background:
                item.id === "VOID"
                  ? "linear-gradient(to left, rgba(124,58,237,1), rgba(192,132,252,0.95), rgba(255,255,255,0.55), transparent)"
                  : item.id === "SOLAR"
                  ? "linear-gradient(to left, rgba(239,68,68,1), rgba(251,146,60,1), rgba(254,240,138,0.95), transparent)"
                  : item.id === "ICE"
                  ? "linear-gradient(to left, rgba(14,165,233,1), rgba(186,230,253,1), rgba(255,255,255,0.75), transparent)"
                  : item.id === "PLASMA"
                  ? "linear-gradient(to left, rgba(236,72,153,1), rgba(34,211,238,1), rgba(255,255,255,0.7), transparent)"
                  : item.id === "RAINBOW"
                  ? "linear-gradient(to left, #ef4444, #f97316, #facc15, #22c55e, #38bdf8, #a78bfa, transparent)"
                  : "linear-gradient(to left, rgba(255,92,24,1), rgba(255,184,76,0.95), rgba(255,255,255,0.55), transparent)",
              filter: "blur(4px)",
              opacity: 0.95,
            }}
          />
          ...
          <div
            style={{
              ...
              background:
                item.id === "VOID"
                  ? "radial-gradient(circle at 28% 24%, #ffffff 0%, #a78bfa 22%, #7c3aed 52%, #1e063f 100%)"
                  : item.id === "SOLAR"
                  ? "radial-gradient(circle at 28% 24%, #fff7ed 0%, #fed7aa 22%, #fb923c 48%, #ef4444 72%, #450a0a 100%)"
                  : item.id === "NEBULA"
                  ? "radial-gradient(circle at 25% 22%, #ffffff 0%, #f0abfc 18%, #ec4899 38%, #06b6d4 68%, #111827 100%)"
                  : item.id === "GOLD"
                  ? "radial-gradient(circle at 28% 22%, #ffffff 0%, #fef08a 20%, #facc15 48%, #b45309 76%, #451a03 100%)"
                  : item.id === "GLITCH"
                  ? "linear-gradient(135deg, #ffffff 0%, #22d3ee 18%, #111827 34%, #f472b6 52%, #22d3ee 68%, #030712 100%)"
                  : "radial-gradient(circle at 28% 24%, rgba(255,255,255,1) 0%, rgba(235,240,248,1) 28%, rgba(170,180,195,1) 62%, rgba(90,100,118,1) 100%)",
              border: "2px solid rgba(255,255,255,0.58)",
              boxShadow: "0 0 20px rgba(255,255,255,0.22)",
            }}
          />
    ...
    <div style={overlayStyle(true, "rgba(0,0,0,0.6)")}>
      <div
        style={{
          ...
          opacity: 0.16,
          animation: "crashFloat 2.8s ease-in-out infinite",
          position: "relative",
        }}
      >
        <div
